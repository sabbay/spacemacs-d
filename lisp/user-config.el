...
;; ----- Auto-render PlantUML blocks when opening plans/*.org ---------------
;; Design / architecture docs live under a `plans/' directory — either
;; project-local (./plans/foo.org) or user-level (~/plans/foo.org). Raw
;; PlantUML source is unreadable; we want the rendered PNG to show up
;; inline the moment the file opens, with no manual C-c C-c.
;;
;; Strategy: on `find-file-hook', detect .org files with a `plans'
;; component in their path, then schedule a one-shot idle timer that
;; walks the buffer and executes every plantuml src block. Using an idle
;; timer keeps file-open snappy — the Java/PlantUML startup (~1s) runs
;; after Emacs is idle instead of blocking the redisplay. True async
;; (make-process) would require bypassing org-babel's synchronous
;; executor; the idle-timer trick gives ~the same UX for v1.

(defun my/org-plans-file-p ()
  "Non-nil iff current buffer visits an .org file with a `plans' path component.
Matches both project-local ./plans/ and user-level ~/plans/. Uses
`file-truename' so symlinked paths (common on macOS) still match."
  (when-let* ((file buffer-file-name)
              (true (ignore-errors (file-truename file))))
    (and (string-match-p "\\.org\\'" true)
         ;; `/plans/' must appear as a full directory component, not a
         ;; substring inside some other name (e.g. `flightplans.org').
         (string-match-p "/plans/" (file-name-directory true)))))

(defun my/org-render-all-plantuml-blocks ()
  "Execute every `#+begin_src plantuml' block in the current buffer.
Errors in one block don't stop the others — each failure just logs a
warning. After execution, refresh inline images so the new PNGs show
up immediately."
  (when (derived-mode-p 'org-mode)
    (let ((count 0) (failed 0))
      (save-excursion
        (goto-char (point-min))
        (while (re-search-forward "^[ \t]*#\\+begin_src[ \t]+plantuml\\b"
                                  nil t)
          (condition-case err
              (progn
            ...
            (error
             (cl-incf failed)
             (message "plantuml auto-render: block at line %d failed: %s"
                      (line-number-at-pos)
                      (error-message-string err))))))
      (when (> count 0)
        (org-redisplay-inline-images))
      (message "plantuml auto-render: %d rendered%s"
               count
               (if (> failed 0) (format ", %d failed" failed) "")))))

(defun my/org-plans-maybe-auto-render ()
  "`find-file-hook': auto-render PlantUML in plans/*.org files.
Defers the actual work to an idle timer so the file-open doesn't
freeze while PlantUML/Java spins up."
  (when (my/org-plans-file-p)
       ...
       (lambda ()
         (when (buffer-live-p buf)
           (with-current-buffer buf
             (my/org-render-all-plantuml-blocks))))))))

(add-hook 'find-file-hook #'my/org-plans-maybe-auto-render)
